...
#' @description Combine all city results of the month, and crop to 16 tiles.
...
#' @export
hp_CHNPost <- function(mon,resol,configfile,outpath,sys,para){
  # make sure the last month, calculation date: Sys.tem()
  if (as.numeric(substr(mon,5,6)) < 12)
  {
    nextmon <- as.character(as.numeric(mon)+1)
  }else{
    nextmon <- as.character(as.numeric(mon)+89)
  }
  # if (nextmon != paste0(substr(Sys.Date(),1,4),substr(Sys.Date(),6,7))){
  #   cat("Please confirm performing your calculation at the beginning of the next month relative to 'mon'!\n")
  #   return(0)
  # }
  ...
  if (!file.exists(paste0(outpath,"/temp/ras_11_newcalprice"))) {dir.create(paste0(outpath,"/temp/ras_11_newcalprice"))}
  if (!file.exists(paste0(outpath,"/temp/ras_11_newlike"))) {dir.create(paste0(outpath,"/temp/ras_11_newlike"))}
  if (!file.exists(paste0(outpath,"/temp/ras_11_newlink"))) {dir.create(paste0(outpath,"/temp/ras_11_newlink"))}
  ...
  if (para){
    if (sys == "wins"){
      ...
      {
        hp_cityPost(cityinfo$pinyinabb[i],cityinfo$host[i],cityinfo$port[i],
                cityinfo$user[i],cityinfo$password[i],cityinfo$dbname[i],
                mon,resol,outpath,sys)
      }
      stopCluster(cl)
    }
  }else{
    for (i in 1:nrow(cityinfo))
    {
      ...
      hp_cityPost(district,host,port,user,password,dbname,mon,resol,outpath,sys)
      cat(as.character(Sys.time()),"\tsucced!\n")
    }
  }
  
  # stop, if no any data
  if (length(dir(paste0(outpath,"/temp/ras_11_newcalprice"))) == 0) {return(0)}
  ...
  for (k in 1:length(vars))
  {
    ...
    for (i in 1:nrow(cityinfo))
    {
      fname <- paste0(outpath,"/temp/ras_11_",vars[k],"/ras_11_",cityinfo$pinyinabb[i],"_",vars[k],"_",mon,".tif")
      ...
      if (sys == "wins"){
        ...
        foreach (tile_i = 1:nrow(tiles)) %dopar% 
        {
          crop(r0,extent(tiles$st_xmin[tile_i],tiles$st_xmax[tile_i],
               ...
               filename=paste0(outpath,"/temp/ras_11_",vars[k],"/ras_11_",tile_i,"_",vars[k],"_",mon,"01.tif"),
               overwrite=TRUE, NAflag=-9999)
        }
        foreach (tile_i = 1:nrow(tiles)) %dopar% 
        {
          swap <- raster(paste0(outpath,"/temp/ras_11_",vars[k],"/ras_11_",tile_i,"_",vars[k],"_",mon,"01.tif"))
          projection(swap) <- CRS("+init=epsg:3857")
          writeRaster(swap,paste0(outpath,"/ras_11_",vars[k],"/ras_11_",tile_i,"_",vars[k],"_",mon,"01.tif"),
